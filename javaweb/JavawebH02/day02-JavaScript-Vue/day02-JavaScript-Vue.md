...
JavaScript 和 Java 是完全不同的语言，不论是概念还是设计。但是基础语法类似。JavaScript 在 1995 年由 Brendan Eich 发明，并于 1997 年成为 ECMA 标准。
...
在css目录同级创建js目录，然后创建一个名为demo.js的文件：



第一步：在demo.js中编写如下js内容：
...
第二步：添加&lt;script&gt;标签来引入外部demo.js文件
...
| 关键字 | 解释                                                         |
| ------ | ------------------------------------------------------------ |
| var    | ECMAScript5中变量声明 全局变量 可以重复定义的                |
...
js中的数据类型分为 ：原始类型 (java基本数据类型)和 引用类型
...
在js中，绝大多数的运算规则和java中是保持一致的

，但是js中的==和===是有区别的。

- ==：只比较值是否相等，不区分数据类型，哪怕类型不一致，==也会自动转换类型进行值得比较(会进行类型转换)
- ===：不光比较值，还要比较类型，如果类型不一致，直接返回false（不会进行类型转换）
...
alert(parseInt("12A45")); //12 不是数字后就停止解析
...
在java中我们为了提高代码的复用性，可以使用方法。同样，在JavaScript中可以使用函数来完成相同的事情。
...
与java中不一样的是，JavaScript中数组相当于java中的集合，数组的长度是可以变化的。而且JavaScript是弱数据类型的语言，所以数组中可以存储任意数据类型的值。接
